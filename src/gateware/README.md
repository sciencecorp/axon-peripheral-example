...
User project generated by `axon-peripheral-sdk new`. Drop your peripheral RTL in,
...
top is a fingerprint that lets `axon-peripheral-sdk regenerate` notice the
edit and refuse to overwrite your work. To discard hand-edits and re-emit
from the manifest, pass `--force-regenerate` (see the `--force` semantics
below).
...
`axon-peripheral-sdk build --pdc devkit` regenerates `src/via_top.sv` + `src/scir_sdk.rdf`
...
- `axon-peripheral-sdk regenerate` — re-emits `via_top.sv` + `scir_sdk.rdf` without
  invoking Radiant. Hand-edits to either file are supported: the checksum
  header is a safety net that lets regenerate notice your edits and refuse
  to overwrite them by default. Pass `--force` (alias `--force-regenerate`)
  when you want to discard hand-edits and re-emit from the manifest. **Note:**
  this checksum safety net only covers the codegen-owned files. Your
  peripheral SystemVerilog and the `# CUSTOMIZE:`-marked regions in test
  files are never overwritten.
- `axon-peripheral-sdk add-peripheral <name>` — appends a new peripheral entry to
  ...
  lands in `via_top.sv`.
- `axon-peripheral-sdk validate` — validates `peripheral.yaml` against the Pydantic
...
- `axon-peripheral-sdk deploy` — packages `build/` outputs into a deployable bundle
  (Debian package or raw bitstream, depending on profile).

## Simulate

`axon-peripheral-sdk sim` runs the cocotb suites listed in each peripheral's
...
broadcast address (`0xFFFF`). `axon-peripheral-sdk validate` enforces the window.
...
- **`src/scir_sdk.pdc` / `src/scir_sdk.sdc`** — board-level constraints
  ...
  touches these after `axon-peripheral-sdk new`.
